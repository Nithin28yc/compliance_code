...
public class Uniqueness extends testBase{
	...
	@FindBy(xpath = "//*[@id=\"DuplicateUser\"]/div[2]/div/div/div/h5")
	public WebElement dulicateUserMsg;
	...
	public boolean addingLdapUser(String username,String password,String ldapuname, String ldapEmail, String ldapPass) throws InterruptedException {
		...
		try {
			...
			if(crt == false){
				...
				if(confirmMsg.getText().equals("Added to Ldap!!!")) {
					logger.log(LogStatus.PASS, "User added with Unique name");
					System.out.println("User added with Unique name");
					ok.click();
					return true;
				}
				else if(dulicateUserMsg.getText().equals("User Name is already added!!!")){
					logger.log(LogStatus.PASS, "User Name is already added!!!");
					System.out.println("User Name is already added!!!");
					return true;
				}
				else {
					logger.log(LogStatus.FAIL, "Uniqueness is not there");
					System.out.println("Uniqueness is not there");
					return false;
				}
			...
			logger.log(LogStatus.FAIL, "Unable to load page");
			System.out.println("Unable to load page");
			Assert.fail("Unable to load page");
			return false;
		}
	}
